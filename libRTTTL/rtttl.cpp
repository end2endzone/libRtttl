...
const TONE_FREQUENCY gRtttlNotes[] = { NOTE_SILENT,
NOTE_C4, NOTE_CS4, NOTE_D4, NOTE_DS4, NOTE_E4, NOTE_F4, NOTE_FS4, NOTE_G4, NOTE_GS4, NOTE_A4, NOTE_AS4, NOTE_B4,
NOTE_C5, NOTE_CS5, NOTE_D5, NOTE_DS5, NOTE_E5, NOTE_F5, NOTE_FS5, NOTE_G5, NOTE_GS5, NOTE_A5, NOTE_AS5, NOTE_B5,
NOTE_C6, NOTE_CS6, NOTE_D6, NOTE_DS6, NOTE_E6, NOTE_F6, NOTE_FS6, NOTE_G6, NOTE_GS6, NOTE_A6, NOTE_AS6, NOTE_B6,
NOTE_C7, NOTE_CS7, NOTE_D7, NOTE_DS7, NOTE_E7, NOTE_F7, NOTE_FS7, NOTE_G7, NOTE_GS7, NOTE_A7, NOTE_AS7, NOTE_B7
};
const size_t gNumRtttlNotes = sizeof(gRtttlNotes)/sizeof(gRtttlNotes[0]);

static const RTTTL_NOTE_CHARACTER gNoteAllowedValue[] =   {'c','d','e','f','g','a','b','p'};
static const int gNoteCharacterCode[] = { 1,  3,  5,  6,  8, 10, 12,  0};
static const size_t gNoteAllowedValueCount = sizeof(gNoteAllowedValue)/sizeof(gNoteAllowedValue[0]);

static const RTTTL_DURATION gDurationAllowedValue[] = {1, 2, 4, 8, 16, MAX_ALLOWED_DURATION_VALUE};
static const size_t gDurationAllowedValueCount = sizeof(gDurationAllowedValue)/sizeof(gDurationAllowedValue[0]);
static const RTTTL_OCTAVE gOctaveAllowedValue[] = {4, 5, 6, 7};
static const size_t gOctaveAllowedValueCount = sizeof(gOctaveAllowedValue)/sizeof(gOctaveAllowedValue[0]);
static const RTTTL_BPM gBpmAllowedValue[] = {25, 28, 31, 35, 40, 45, 50, 56, 63, 70, 80, 90, 100, 112, 125, 140, 160, 180, 200, 225, 250, 285, 320, 355, 400, 450, 500, 565, 635, 715, 800, 900};
static const size_t gBpmAllowedValueCount = sizeof(gBpmAllowedValue)/sizeof(gBpmAllowedValue[0]);
...
RTTTL_NOTE_CHARACTER getNoteCharacterFromIndex(NOTE_CHARACTER_INDEX iIndex)
{
  if (iIndex >= 0 && iIndex < gNoteAllowedValueCount)
    return gNoteAllowedValue[iIndex];
  return -1;
}

size_t getNoteCharactersCount()
{
  return gNoteAllowedValueCount;
}

NOTE_CHARACTER_INDEX findNoteCharacterIndex(RTTTL_NOTE_CHARACTER n)
{
  for(NOTE_CHARACTER_INDEX i=0; i<gNoteAllowedValueCount; i++)
  {
    if (getNoteCharacterFromIndex(i) == n)
    {
      return i;
    }
  }
...
{
  if (iIndex >= 0 && iIndex < gDurationAllowedValueCount)
    return gDurationAllowedValue[iIndex];
  return -1;
}

size_t getNoteDurationsCount()
{
  return gDurationAllowedValueCount;
}

DURATION_INDEX findNoteDurationIndex(RTTTL_DURATION n)
{
  for(DURATION_INDEX i=0; i<gDurationAllowedValueCount; i++)
  {
...
RTTTL_OCTAVE getNoteOctaveFromIndex(OCTAVE_INDEX iIndex)
{
  if (iIndex >= 0 && iIndex < gOctaveAllowedValueCount)
    return gOctaveAllowedValue[iIndex];
  return -1;
}

size_t getNoteOctavesCount()
{
  return gOctaveAllowedValueCount;
}

OCTAVE_INDEX findNoteOctaveIndex(RTTTL_OCTAVE n)
{
  for(OCTAVE_INDEX i=0; i<gOctaveAllowedValueCount; i++)
  {
...
RTTTL_BPM getOfficialBpmFromIndex(BPM_INDEX iIndex)
{
  if (iIndex >= 0 && iIndex < gBpmAllowedValueCount)
    return gBpmAllowedValue[iIndex];
  return -1;
}

size_t getOfficialBpmCount()
{
  return gBpmAllowedValueCount;
}

BPM_INDEX findOfficialBpmIndex(RTTTL_BPM n)
{
  for(BPM_INDEX i=0; i<gBpmAllowedValueCount; i++)
  {
    if (getOfficialBpmFromIndex(i) == n)
    {
      return i;
    }
  }
...
void reset(RTTTL_DEFAULT_VALUE_SECTION & iDefaults)
{
  ...
  {
    iDefaults.anyBpm = RTTTL_DEFAULT_BPM;
    iDefaults.hasAnyBpm = true;
  }
  else
  {
    iDefaults.bpmIdx = RTTTL_DEFAULT_BPM_INDEX;
  }
}
...
std::string toString(const RTTTL_NOTE & iNote)
{
  ...
  if (getNoteCharacterFromIndex(iNote.noteIdx) == 'p')
  {
    sprintf(buffer, "%d%c%s", 
      getNoteDurationFromIndex(iNote.durationIdx),
      getNoteCharacterFromIndex(iNote.noteIdx),
      (iNote.dotted ? "." : "")
      );
  }
  else
...
std::string toString(const RTTTL_NOTE & iNote, const RTTTL_DEFAULT_VALUE_SECTION & iDefaults)
{
  ...
  //note character
  note.append( toString(getNoteCharacterFromIndex(iNote.noteIdx)) );
  ...
  //octave
  if (iDefaults.octaveIdx != iNote.octaveIdx && getNoteCharacterFromIndex(iNote.noteIdx) != 'p')
  {
...
std::string toString(const RTTTL_DEFAULT_VALUE_SECTION & iDefaults, bool iForceIncludeDefaults)
{
  ...
  if (iDefaults.hasAnyBpm)
  {
    if (iForceIncludeDefaults || iDefaults.anyBpm != RTTTL_DEFAULT_BPM)
    {
      if (defaultSection != "")
        defaultSection.append(",");
      defaultSection.append("b=" + itostr(iDefaults.anyBpm));
    }
  }
  else
  {
    if (iForceIncludeDefaults || iDefaults.bpmIdx != RTTTL_DEFAULT_BPM_INDEX)
    {
      if (defaultSection != "")
        defaultSection.append(",");
      defaultSection.append("b=" + itostr( getOfficialBpmFromIndex(iDefaults.bpmIdx) ));
    }
  }
...
void sortBpms(BpmList & ioBpms)
{
  struct BpmSorter
  {
    ...
    {
      BPM_INDEX index1 = findOfficialBpmIndex(bpm1);
      BPM_INDEX index2 = findOfficialBpmIndex(bpm2);
...
{
  BPM_INDEX bpmIdx = findOfficialBpmIndex(iBpm);
  if ( 1 > iBpm && iBpm > 900 )
  {
  ...
  {
    //not an official bpm
    ioDefaults.anyBpm = iBpm;
    ioDefaults.hasAnyBpm = true;
  }
  else if ( bpmIdx != INVALID_BPM_INDEX )
  {
    ioDefaults.bpmIdx = bpmIdx;
    ioDefaults.hasAnyBpm = false;
  }
  else
  {
    return UNKNOWN_BPM;
...
LIBRTTTL_API RTTTL_RESULT parseRtttlString(const char * iCode, RTTTL_SONG & oSong)
{
  ...
  RTTTL_OCTAVE default_oct = RTTTL_DEFAULT_OCTAVE;
  ...
  while(*p)
  {
    ...
    // now get the note
    int noteIndex = findNoteCharacterIndex(*p);
    if (noteIndex == -1)
    {
      note.noteIdx = findNoteCharacterIndex('p');
    }
    else
    {
...
size_t findRtttlNoteIndex(TONE_FREQUENCY iFrequency, TONE_FREQUENCY iEpsilon)
{
  ...
  for(size_t i=0; i<gNumRtttlNotes; i++)
  {
    TONE_FREQUENCY noteFreq = gRtttlNotes[i];
...
size_t findClosestRtttlNoteIndex(TONE_FREQUENCY iFrequency)
{
  TONE_FREQUENCY gRtttlNotes_delta[gNumRtttlNotes] = {0};

  //compute delta
  for(int i=0; i<gNumRtttlNotes; i++)
  {
    TONE_FREQUENCY noteFreq = gRtttlNotes[i];
...
RTTTL_DURATION_RESULT findDurationIndex(RTTTL_BPM iBpm, TONE_DURATION iToneDuration)
{
  ...
  for(DURATION_INDEX durationIdx = 0; durationIdx < gDurationAllowedValueCount; durationIdx++)
  {
...
BpmList findValidBpms(TONE_DURATION iDuration)
{
  ...
  {
    for(BPM_INDEX bpmIdx = 0; bpmIdx<gBpmAllowedValueCount; bpmIdx++)
    {
      RTTTL_BPM bpm = getOfficialBpmFromIndex(bpmIdx);
      RTTTL_DURATION_RESULT result = findDurationIndex(bpm, iDuration);
      if (result.durationIdx != INVALID_DURATION_INDEX)
      {
        bpms.push_back(bpm);
...
RTTTL_BPM findBestBpm(const TONE_CALL_INFO_LIST & iCalls)
{
  ...
  for(size_t i=0; i<allBpm.size(); i++)
  {
    ...
    for(BPM_INDEX j=0; j<gBpmAllowedValueCount; j++)
    {
      RTTTL_BPM allowedBpm = gBpmAllowedValue[j];
...
OctaveList getSongOctavesList(const RTTTL_NOTE_LIST & iNotes)
{
  ...
  int count[gOctaveAllowedValueCount] = {0}; //from 0 to 3

  for(size_t i=0; i<iNotes.size(); i++)
  {
    const RTTTL_NOTE & note = iNotes[i];
    count[note.octaveIdx]++;
  }

  //find used octave
  OctaveList octaves;
  for(OCTAVE_INDEX octaveIdx=0; octaveIdx<gOctaveAllowedValueCount; octaveIdx++)
  {
    int octaveCount = count[octaveIdx];
    if (octaveCount > 0)
    {
...
RTTTL_OCTAVE getBestOctaveOptimization(const RTTTL_NOTE_LIST & iNotes)
{
  //find the best values for octave by looking at all the notes
  int count[gOctaveAllowedValueCount] = {0}; //from 0 to 3
  ...
  size_t popularOctaveIndex = findMaxValueIndex(count, sizeof(count)/sizeof(count[0]));
  return (RTTTL_OCTAVE)popularOctaveIndex;
}
...
DurationList getSongDurationsList(const RTTTL_NOTE_LIST & iNotes)
{
  ...
  int count[gDurationAllowedValueCount] = {0};

  for(size_t i=0; i<iNotes.size(); i++)
  {
    const RTTTL_NOTE & note = iNotes[i];
    count[note.durationIdx]++;
  }

  //find used octave
  DurationList durations;
  for(DURATION_INDEX durationIdx=0; durationIdx<gOctaveAllowedValueCount; durationIdx++)
  {
    int durationCount = count[durationIdx];
    if (durationCount > 0)
    {
...
RTTTL_DURATION getBestDurationOptimization(const RTTTL_NOTE_LIST & iNotes)
{
  ...
  size_t popularDurationIndex = findMaxValueIndex(count, sizeof(count)/sizeof(count[0]));
...
RTTTL_NOTE getRtttlNote(RTTTL_BPM iBpm, TONE_FREQUENCY iFrequency, TONE_DURATION iDuration) //iDuration in ms
{
  ...
  size_t noteFrequencyIndex = findRtttlNoteIndex(iFrequency, TONE_FREQUENCY_EPSILON);

  //if a perfect match not found, find the closest note possible.
  if (noteFrequencyIndex == (size_t)-1)
    noteFrequencyIndex = findClosestRtttlNoteIndex(iFrequency);

  if (noteFrequencyIndex == 0)
  {
    n.noteIdx = findNoteCharacterIndex('p');
  }
  else
  {
    ...
    for(NOTE_CHARACTER_INDEX noteIdx = 0; noteIdx<gNoteAllowedValueCount; noteIdx++)
    {
      int noteCharacterCode = gNoteCharacterCode[noteIdx];
      for(OCTAVE_INDEX octaveIdx = 0; octaveIdx < gOctaveAllowedValueCount; octaveIdx++)
      {
        RTTTL_OCTAVE scale = gOctaveAllowedValue[octaveIdx];

        //evaluate
        int tmp = ((scale - 4) * 12 + noteCharacterCode);
    ...
    if (!noteFound)
    {
      for(NOTE_CHARACTER_INDEX noteIdx = 0; noteIdx<gNoteAllowedValueCount; noteIdx++)
      {
        ...
        for(OCTAVE_INDEX octaveIdx = 0; octaveIdx < gOctaveAllowedValueCount; octaveIdx++)
        {
          ...
          //evaluate
          int tmp = ((scale - 4) * 12 + (noteCharacterCode) + 1);
          if (tmp == noteFrequencyIndex)
          {
            //found!
            n.noteIdx = noteIdx;
...
void _playRTTTL(const char *p)
{
  ...
  while(*p)
  {
    ...
    if(note)
    {
      tone(BUZZER_OUT_PIN, gRtttlNotes[(scale - 4) * 12 + note], duration);
      delay(duration+1);
...
void _playRTTTL(const RTTTL_SONG & iSong)
{
  ...
  // BPM usually expresses the number of quarter notes per minute
  if (iSong.defaults.hasAnyBpm)
  {
    wholenote = (60 * 1000L / iSong.defaults.anyBpm) * 4;  // this is the time for whole note (in milliseconds)
  }
  else
  {
    wholenote = (60 * 1000L / getOfficialBpmFromIndex(iSong.defaults.bpmIdx)) * 4;  // this is the time for whole note (in milliseconds)
  }
  ...
  for(size_t i=0; i<iSong.notes.size(); i++)
  {
    ...
    // now get the note
    note = gNoteCharacterCode[n.noteIdx];
