...
void testRTTTLConversion(const char * iRtttlString)
{
  ...
  printf("Testing: %s, song name = %s... ", __FUNCTION__, songName.c_str());
...
void testParseRTTTL(const char * iRtttlString)
{
  ...
  printf("Testing: %s, %s... ", __FUNCTION__, songName.c_str());
...
void testImperialMarchRTTTLConvertion()
{
...
void testImperialMarchRTTTLConvertionFailure()
{
  printf("Testing: %s... ", __FUNCTION__);
...
void testToCppArray()
{
  ...
  ASSERT_TRUE(inputStream.buffer);
...
void testSizeof()
{
  ...
  //test size
  printf("sizeof(RTTTL_DEFAULT_VALUE_SECTION)=%d, sizeof(RTTTL_NOTE)=%d. ", 
...
void testFindAlternateRtttlEncoding()
{
  printf("Testing: %s... ", __FUNCTION__);
...
int _tmain(int argc, _TCHAR* argv[])
{
  ...
  testBitReaderCopy16();
  testBitAddress();
  //generateBinaryDefines("binary.h");
  //generate5BitCounterStream("counter5bits.h");
  test5BitCounterRead();
  test5BitCounterWrite();
  testRTTTLConversion(zelda);
  testRTTTLConversion(simpsons);
  testParseRTTTL(zelda);
  testParseRTTTL(simpsons);
  testPlayRTTTL(zelda);
  testImperialMarchRTTTLConvertion();
  testImperialMarchRTTTLConvertionFailure();
  testComputeEpsilonValues();
  testParseToneCallsFile();
  testToCppArray();
  testParseRtttlStream();
  testSizeof();
  testFindAlternateRtttlEncoding();
  testOptimizeEncoding();
